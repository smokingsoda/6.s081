...
void usertrap(void) {
    ...
    if (r_scause() == 8) {
        ...
        // handle the lazy allocate page fault in VMA
        uint64 addr = r_stval();
        uint index;
        uint64 alloc_addr;
        uint64 pages = 0;
    ...
    vma_page_fault:
        // allocate pages, set their flags
        ...
        while (pages < PGROUNDUP(p->vma[index].length)) {
            ...
            printf("file %p is mmaping va %p to pa %p\n", p->vma[index].fp,
                   vma->addr + pages, alloc_addr);
